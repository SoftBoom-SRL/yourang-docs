...
## Actions and Classic views <a id="tool-views"></a>

In Tools, choose Actions or Classic. This browser remembers the preference without changing the agent configuration. Actions groups tools by category; typing in search searches all categories in the current panel, ignoring status filters. Open Details to explore conversation scenarios or access configuration. Scenarios are illustrative guidance: they do not execute tools or guarantee agent behavior. For external tools, users with management permissions can open real history and real testing; a test can affect the connected service. An active action does not mean its connection has been verified.
